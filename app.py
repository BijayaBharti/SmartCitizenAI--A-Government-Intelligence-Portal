...
UI_LABELS = {
    "English": {
        ...
        "hero_badge_3": "🛡️ Privacy-Aware", "hero_badge_4": "🕘 24/7 Available",
...
with st.sidebar:
    ...
    selected_dark_mode = st.toggle(
        t("theme_label"), value=st.session_state.dark_mode, key="sidebar_dark_mode"
    )
...
def load_schemes():
    """Load the optional schemes.json file without crashing the whole app.

    The project can run without the external scheme file; in that case the Scheme
    Finder shows a clear message instead of raising FileNotFoundError.
    """
...
def get_tavily_tool(max_results=5):
    """Create Tavily tool using the environment variable supported across LangChain versions."""
    if not TAVILY_IMPORT_OK or not TAVILY_API_KEY:
        return None
    os.environ["TAVILY_API_KEY"] = TAVILY_API_KEY
...
def safe_filename(filename, default="document.pdf"):
    """Return a filesystem-safe basename for uploaded/generated files."""
...
def run_text_extract_on_pdf(file_bytes, filename):
    ...
    safe_name = safe_filename(filename, "document.pdf")
    temp_path = os.path.join(PDF_SAVE_DIR, f"ocr_{safe_name}")
...
def load_and_split_pdf(file_bytes, filename):
    if not os.path.exists(PDF_SAVE_DIR):
        os.makedirs(PDF_SAVE_DIR)
    safe_name = safe_filename(filename, "document.pdf")
    file_path = os.path.join(PDF_SAVE_DIR, safe_name)
...
#===========================================================
def filter_schemes(age, annual_income, selected_categories, state=None):
    schemes = load_schemes()
    matches = []
    selected = set(selected_categories or [])
    for scheme in schemes:
        try:
            if not (int(scheme.get("min_age", 0)) <= age <= int(scheme.get("max_age", 120))):
                continue
            if annual_income > float(scheme.get("max_income", float("inf"))):
                continue
            categories = set(scheme.get("categories", []))
            if selected and not (selected & categories):
                continue

            # If the dataset provides state/state-specific fields, respect them.
            # If it does not, keep the scheme eligible instead of breaking older data.
            if state and state != "Other":
                scheme_states = scheme.get("states", scheme.get("state", None))
                if scheme_states:
                    if isinstance(scheme_states, str):
                        scheme_states = [scheme_states]
                    normalized = {str(x).strip().lower() for x in scheme_states}
                    if "all" not in normalized and state.strip().lower() not in normalized:
                        continue

            matches.append(scheme)
        except (TypeError, ValueError):
            # Ignore malformed records instead of crashing the complete Scheme Finder.
            continue
    return matches
    ...
    if TAVILY_IMPORT_OK and TAVILY_API_KEY:
        try:
            ...
            if isinstance(updates, dict):
                updates = updates.get("results", [updates])
...
def render_page_scheme():
    ...
    if submitted:
        if not load_schemes():
            st.session_state.scheme_matches = []
            st.session_state.scheme_advice = None
            st.warning("Scheme data file (schemes.json) is missing or invalid. Add a valid schemes.json next to app.py to enable the Scheme Finder.")
            return
        matches = filter_schemes(age, annual_income, categories, state)
    ...
    if st.button(t("search_btn")) and search_query:
        if TAVILY_IMPORT_OK and TAVILY_API_KEY:
            with st.spinner(t("search_searching")):
                try:
                    search_tool = get_tavily_tool(max_results=5)
                    if search_tool is None:
                        raise RuntimeError("Tavily is not configured. Add TAVILY_API_KEY in Settings.")
                    results = search_tool.invoke({"query": search_query})
                    if isinstance(results, str):
                        try:
                            results = json.loads(results)
                        except json.JSONDecodeError:
                            results = [{"title": "Search Result", "url": "#", "content": results}]
...
def render_page_translator():
    ...
    with tc2:
        target_lang = st.selectbox(
            t("translator_target"), ["English", "Hindi", "Hinglish (Roman Hindi)"], key="translator_target_lang"
        )
        translate_btn = st.button(t("translator_btn"), key="translator_submit_btn")
